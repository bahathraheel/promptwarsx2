/**
 * ASCIIText - Vanilla JS Port
 * Adapted from React Bits <ASCIIText />
 */
...
class CanvasTxt {
  ...
  resize() {
    ...
    let textHeight = this.fontSize * 1.5;
    if (metrics.actualBoundingBoxAscent) {
      textHeight = Math.ceil(metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent) + 20;
    }
...
class ASCIIText {
  ...
  async init() {
    // Inject the CSS
    if (!document.getElementById("ascii-text-css")) {
      const style = document.createElement("style");
      style.id = "ascii-text-css";
      style.textContent = `
        ...
        .ascii-pre {
          ...
          top: 0;
          color: #4df8ff;
          text-shadow: 0 0 10px rgba(77, 248, 255, 0.5);
          z-index: 9;
        }
      `;
    ...
    this.container.style.pointerEvents = "none"; // Don't block background clicks
    ...
    this.instance = new CanvAscii(this.options, this.container, w, h);
