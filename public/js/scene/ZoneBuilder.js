...
class ZoneBuilder {
  ...
  _buildIdCard() {
    ...
    const boardLines = new THREE.LineSegments(
      boardEdges,
      new THREE.LineBasicMaterial({
        color: 0xffaa44,
        transparent: true,
  ...
  _buildCalendar() {
    ...
    const calMat = new THREE.MeshPhongMaterial({
      color: 0xeeeeee,
      flatShading: true,
    });
    ...
    const calLines = new THREE.LineSegments(
      calEdges,
      new THREE.LineBasicMaterial({
        color: 0x8844ff,
        transparent: true,
    ...
    const headerMat = new THREE.MeshPhongMaterial({
      color: 0xdd3344,
      flatShading: true,
    });
  ...
  _buildBallotBox() {
    ...
    const ballotMat = new THREE.MeshPhongMaterial({
      color: 0xffffff,
      flatShading: true,
    });
  ...
  _buildPodium() {
    ...
    const trophyMat = new THREE.MeshPhongMaterial({
      color: 0xffdd00,
      emissive: 0xaa8800,
      flatShading: true,
    });
  ...
  animate(elapsed) {
    ...
    if (this.zones.polling) {
      const ballot = this.zones.polling.children.find(
        (c) => c.userData && c.userData.floats,
      );
      if (ballot) {
