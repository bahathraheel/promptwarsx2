/**
 * Infinite Menu (Vanilla JS Port)
 * Adapted from React Bits <InfiniteMenu />
 */
...
class ArcballControl {
  isPointerDown = false;
  orientation = glMatrix.quat.create();
  pointerRotation = glMatrix.quat.create();
  rotationVelocity = 0;
  rotationAxis = glMatrix.vec3.fromValues(1, 0, 0);
  snapDirection = glMatrix.vec3.fromValues(0, 0, -1);
  snapTargetDirection;
  EPSILON = 0.1;
  IDENTITY_QUAT = glMatrix.quat.create();
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      const midPointerPos = glMatrix.vec2.sub(
    ...
    const combinedQuat = glMatrix.quat.multiply(
      glMatrix.quat.create(),
      snapRotation,
      this.pointerRotation,
    );
    this.orientation = glMatrix.quat.multiply(
      glMatrix.quat.create(),
      combinedQuat,
      this.orientation,
    );
...
class InfiniteGridMenu {
  ...
  resize() {
    this.viewportSize = glMatrix.vec2.set(
  ...
  #init(onInit) {
    ...
    this.viewportSize = glMatrix.vec2.fromValues(
      this.canvas.clientWidth,
      this.canvas.clientHeight,
    );
    ...
    positions.forEach((p, ndx) => {
      ...
      glMatrix.mat4.multiply(
        ...
        glMatrix.mat4.fromTranslation(
          ...
          glMatrix.vec3.negate(glMatrix.vec3.create(), p),
        ),
      );
      glMatrix.mat4.multiply(
        matrix,
        matrix,
        glMatrix.mat4.targetTo(glMatrix.mat4.create(), [0, 0, 0], p, [0, 1, 0]),
      );
      glMatrix.mat4.multiply(
        matrix,
        matrix,
        glMatrix.mat4.fromScaling(glMatrix.mat4.create(), [
          finalScale,
          finalScale,
          finalScale,
        ]),
      ...
      glMatrix.mat4.multiply(
        matrix,
        matrix,
        glMatrix.mat4.fromTranslation(glMatrix.mat4.create(), [
          0,
          0,
          -this.SPHERE_RADIUS,
        ]),
  ...
  #findNearestVertexIndex() {
    ...
    const inversOrientation = glMatrix.quat.conjugate(
      glMatrix.quat.create(),
      this.control.orientation,
    );
    const nt = glMatrix.vec3.transformQuat(
      glMatrix.vec3.create(),
      n,
      inversOrientation,
    );
  ...
  #getVertexWorldPosition(index) {
    ...
    return glMatrix.vec3.transformQuat(
      glMatrix.vec3.create(),
      nearestVertexPos,
      this.control.orientation,
    );
...
/**
 * Wrapper for the Vanilla JS DOM setup
 */
class InfiniteMenu {
  constructor(container, items = [], scale = 1.0) {
  ...
  render() {
    ...
    this.canvas.id = "infinite-grid-menu-canvas";
    ...
    this.overlay.className = "infinite-menu-overlay";

    this.titleEl = document.createElement("h2");
    this.titleEl.className = "face-title";

    this.descEl = document.createElement("p");
    this.descEl.className = "face-description";

    this.btnEl = document.createElement("div");
    this.btnEl.className = "action-button";
    this.btnEl.innerHTML = '<p class="action-button-icon">&#x2197;</p>';
    this.btnEl.addEventListener("click", () => {
      if (!this.activeItem?.link) return;
      if (this.activeItem.link.startsWith("http")) {
        window.open(this.activeItem.link, "_blank");
      } else {
        console.log("Internal route:", this.activeItem.link);
      }
    });
  ...
  initWebGL() {
    ...
    const sketch = new InfiniteGridMenu(
      this.canvas,
      this.items.length ? this.items : defaultItems,
      (index) => this.handleActiveItem(index),
      (isMoving) => this.handleMovementChange(isMoving),
      (sk) => sk.run(),
      this.scale,
    );
  ...
  updateDOM() {
    ...
    this.titleEl.textContent = this.activeItem.title;
    this.descEl.textContent = this.activeItem.description;

    const stateClass = this.isMoving ? "inactive" : "active";

    this.titleEl.className = `face-title ${stateClass}`;
    this.descEl.className = `face-description ${stateClass}`;
    this.btnEl.className = `action-button ${stateClass}`;
  }
}
