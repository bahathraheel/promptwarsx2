    ...
    try {
      ...
      if (data.data.timeline) {
        ...
        const entries = Object.entries(data.data.timeline)
          .map(
            ([key, val]) =>
              `<li><strong>${key.replace(/_/g, " ")}:</strong> ${val}</li>`,
          )
          .join("");
      ...
      container.innerHTML =
        '<p style="color:var(--elite-danger);">Failed to load content. Please refresh or visit <a href="https://vote.gov">vote.gov</a> for election information.</p>';
    }
  }
