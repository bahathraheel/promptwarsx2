...
const askValidation = [
  body('question')
    ...
    .trim()
    .isLength({ min: 2, max: 500 }).withMessage('Question must be 2-500 characters')
    .escape(),
  body('zoneId')
    .optional()
    .isString()
    .matches(/^[a-zA-Z0-9_-]+$/).withMessage('Invalid zone ID format'),
  body('language')
    .optional()
    .isString()
...
const translateValidation = [
  body('text')
    ...
    .trim()
    .isLength({ min: 1, max: 2000 }).withMessage('Text must be 1-2000 characters'),
  body('targetLanguage')
    .isString().withMessage('Target language is required')
...
const ttsValidation = [
  body('text')
    .isString().withMessage('Text must be a string')
    .trim()
    .isLength({ min: 1, max: 1000 }).withMessage('Text must be 1-1000 characters'),
  body('languageCode')
    .optional()
    .isString()
    .matches(/^[a-z]{2}(-[A-Z]{2})?$/).withMessage('Invalid language code'),
  validate
];
...
const zoneIdValidation = [
  param('id')
    .isString()
    .matches(/^[a-zA-Z0-9_-]+$/).withMessage('Invalid zone ID'),
  validate
];
