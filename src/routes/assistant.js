...
const { askAssistant, getProactiveTip, generateFollowUps, classifyIntent } = require('../services/gemini');
const { synthesizeSpeech } = require('../services/tts');
const { analyzeSentiment } = require('../services/nlp');
const { translateText } = require('../services/translate');
const { askValidation, ttsValidation, sentimentValidation } = require('../middleware/validation');
...
router.post('/ask', assistantLimiter, askValidation, async (req, res, next) => {
  try {
    ...
    const [aiResponse, sentiment] = await Promise.all([
      askAssistant(question, zoneId, language || 'en', conversationHistory || []),
      analyzeSentiment(question).catch(() => ({ score: 0, magnitude: 0, fallback: true }))
    ]);
    ...
    if (language && language !== 'en') {
      try {
        const translated = await translateText(aiResponse.answer, language, 'en');
    ...
    res.json({
      ...
      data: {
        ...
        originalAnswer: language && language !== 'en' ? aiResponse.answer : undefined,
